...
Reads storm forecasts, writes per-run history to storm_risk_log. NTFY/Telegram ping on alertable risk (rank ≥ 4 = Slight or higher).
...
# Run the agent
cd /root/empire-v49
/usr/bin/python3 -m agents.warp_scout 2>&1 | tail -20
```
...
- NTFY/Telegram pings disabled (creds not configured). Logs to agent_activity only.
- Storm risk is currently Wichita Slight (rank 3) + KC Enhanced (rank 4). No real storm activity in next 48h for our metros.
