"""Apply QC dashboard edits to empire_command_spa.py - Part 2 (CSS + component)."""

with open('empire_command_spa.py', 'r') as f:
    content = f.read()

# Read exact line 1007 to get the correct characters
import subprocess
line_1007 = subprocess.check_output(['sed', '-n', '1008p', 'empire_command_spa.py']).decode('utf-8').strip()
print(f"Line 1007 repr: {repr(line_1007)}")

# Find the COMMAND CENTER PRO CSS section anchor
# Use a more flexible match: look for ".ccp-dash{padding:0 4px}" 
old3 = ".ccp-dash{padding:0 4px}"
count3 = content.count(old3)
print(f"Edit 3 anchor: found {count3} occurrences of '.ccp-dash{{padding:0 4px}}'")

# Get the exact start of the CCP CSS section
idx = content.find("/* ── COMMAND CENTER PRO ─")
if idx < 0:
    # Try finding with just text
    idx = content.find("COMMAND CENTER PRO")
    if idx >= 0:
...
if idx < 0:
    print("ERROR: Could not find COMMAND CENTER PRO section")
    exit(1)

# Get the exact old string from the file
end_marker = ".ccp-summary-car"  # this should be unique
end_idx = content.find(end_marker, idx)
if end_idx < 0:
    print("ERROR: Could not find end marker")
    exit(1)

# Go back to find ".ccp-dash{padding:0 4px}"
dash_idx = content.find(".ccp-dash{padding:0 4px}", idx, end_idx)
if dash_idx < 0:
    print("ERROR: Could not find .ccp-dash")
    exit(1)

# Extract the old string - from the comment start to end of .ccp-dash line
old3_actual = content[idx:dash_idx + len(".ccp-dash{padding:0 4px}")]
print(f"Old3 actual (first 80 chars): {repr(old3_actual[:80])}")
...
new3 = old3_actual.replace(".ccp-dash{padding:0 4px}", qc_css + ".ccp-dash{padding:0 4px}")
content = content.replace(old3_actual, new3, 1)
